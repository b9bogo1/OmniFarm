import enum
from datetime import datetime, date, timezone
from app.db import get_col, oid, date_to_dt, dt_to_date


class FlockStatus(enum.Enum):
    ACTIVE = 'active'
    SOLD   = 'sold'
    CULLED = 'culled'


class Flock:
    def __init__(self, doc=None, **kw):
        doc = {**(doc or {}), **kw}
        object.__setattr__(self, '_id', doc.get('_id'))
        self.name           = doc.get('name', '')
        self.species        = doc.get('species', 'Poulet de chair')
        self.breed          = doc.get('breed')
        self.placement_date = dt_to_date(doc.get('placement_date'))
        self.initial_count  = doc.get('initial_count', 0)
        self.house_number   = doc.get('house_number')
        _s = doc.get('status', FlockStatus.ACTIVE.value)
        self.status = FlockStatus(_s.lower()) if isinstance(_s, str) else _s
        self.notes          = doc.get('notes')
        self.created_at     = doc.get('created_at', datetime.now(timezone.utc))
        self._record_count   = None
        self._total_mortality = None
        self._total_eggs     = None
        self._latest_record  = None

    @property
    def id(self):
        return str(self._id) if self._id else None

    @property
    def age_days(self) -> int | None:
        return (date.today() - self.placement_date).days if self.placement_date else None

    @property
    def record_count(self) -> int:
        if self._record_count is not None:
            return self._record_count
        return get_col('poultry_records').count_documents({'flock_id': self.id})

    @property
    def total_mortality(self) -> int:
        if self._total_mortality is not None:
            return self._total_mortality
        result = list(get_col('poultry_records').aggregate([
            {'$match': {'flock_id': self.id}},
            {'$group': {'_id': None, 'total': {'$sum': '$mortality_count'}}},
        ]))
        return result[0]['total'] if result else 0

    @property
    def total_eggs(self) -> int:
        if self._total_eggs is not None:
            return self._total_eggs
        result = list(get_col('poultry_records').aggregate([
            {'$match': {'flock_id': self.id}},
            {'$group': {'_id': None, 'total': {'$sum': '$eggs_collected'}}},
        ]))
        return result[0]['total'] if result else 0

    @property
    def latest_record(self):
        if self._latest_record is not None:
            return self._latest_record
        doc = get_col('poultry_records').find_one(
            {'flock_id': self.id}, sort=[('record_date', -1)]
        )
        return PoultryRecord(doc) if doc else None

    def save(self) -> 'Flock':
        col = get_col('flocks')
        doc = self._to_doc()
        if self._id:
            col.replace_one({'_id': self._id}, doc)
        else:
            result = col.insert_one(doc)
            object.__setattr__(self, '_id', result.inserted_id)
        return self

    def delete(self) -> None:
        if self._id:
            get_col('flocks').delete_one({'_id': self._id})
            get_col('poultry_records').delete_many({'flock_id': self.id})

    def _to_doc(self) -> dict:
        return {
            'name':           self.name,
            'species':        self.species,
            'breed':          self.breed,
            'placement_date': date_to_dt(self.placement_date),
            'initial_count':  self.initial_count,
            'house_number':   self.house_number,
            'status':         self.status.value,
            'notes':          self.notes,
            'created_at':     self.created_at,
        }

    @classmethod
    def get_by_id(cls, id_str) -> 'Flock | None':
        doc = get_col('flocks').find_one({'_id': oid(id_str)})
        return cls(doc) if doc else None

    @classmethod
    def find_all(cls) -> list['Flock']:
        return [cls(d) for d in get_col('flocks').find().sort('created_at', -1)]

    @classmethod
    def count_by_status(cls, status: FlockStatus) -> int:
        return get_col('flocks').count_documents({'status': status.value})

    def __repr__(self) -> str:
        return f'<Flock {self.name!r}>'


class PoultryRecord:
    def __init__(self, doc=None, **kw):
        doc = {**(doc or {}), **kw}
        object.__setattr__(self, '_id', doc.get('_id'))
        self.flock_id         = doc.get('flock_id')
        self.recorded_by_id   = doc.get('recorded_by_id')
        self.record_date      = dt_to_date(doc.get('record_date'))
        self.feed_quantity_kg = doc.get('feed_quantity_kg')
        self.feed_type        = doc.get('feed_type')
        self.water_consumed_l = doc.get('water_consumed_l')
        self.eggs_collected   = doc.get('eggs_collected', 0)
        self.avg_weight_g     = doc.get('avg_weight_g')
        self.mortality_count  = doc.get('mortality_count', 0)
        self.mortality_cause  = doc.get('mortality_cause')
        self.ambient_temp_c   = doc.get('ambient_temp_c')
        self.humidity_pct     = doc.get('humidity_pct')
        self.notes            = doc.get('notes')
        self.created_at       = doc.get('created_at', datetime.now(timezone.utc))

    @property
    def id(self):
        return str(self._id) if self._id else None

    @property
    def flock(self) -> 'Flock | None':
        return Flock.get_by_id(self.flock_id) if self.flock_id else None

    def save(self) -> 'PoultryRecord':
        col = get_col('poultry_records')
        doc = self._to_doc()
        if self._id:
            col.replace_one({'_id': self._id}, doc)
        else:
            result = col.insert_one(doc)
            object.__setattr__(self, '_id', result.inserted_id)
        return self

    def delete(self) -> None:
        if self._id:
            get_col('poultry_records').delete_one({'_id': self._id})

    def _to_doc(self) -> dict:
        return {
            'flock_id':         self.flock_id,
            'recorded_by_id':   self.recorded_by_id,
            'record_date':      date_to_dt(self.record_date),
            'feed_quantity_kg': self.feed_quantity_kg,
            'feed_type':        self.feed_type,
            'water_consumed_l': self.water_consumed_l,
            'eggs_collected':   self.eggs_collected or 0,
            'avg_weight_g':     self.avg_weight_g,
            'mortality_count':  self.mortality_count or 0,
            'mortality_cause':  self.mortality_cause,
            'ambient_temp_c':   self.ambient_temp_c,
            'humidity_pct':     self.humidity_pct,
            'notes':            self.notes,
            'created_at':       self.created_at,
        }

    @classmethod
    def get_by_id(cls, id_str) -> 'PoultryRecord | None':
        doc = get_col('poultry_records').find_one({'_id': oid(id_str)})
        return cls(doc) if doc else None

    @classmethod
    def find_by_flock(cls, flock_id_str: str, asc=True) -> list['PoultryRecord']:
        order = 1 if asc else -1
        docs = get_col('poultry_records').find(
            {'flock_id': flock_id_str}
        ).sort('record_date', order)
        return [cls(d) for d in docs]

    @classmethod
    def count_all(cls) -> int:
        return get_col('poultry_records').count_documents({})

    def __repr__(self) -> str:
        return f'<PoultryRecord flock={self.flock_id} date={self.record_date}>'
